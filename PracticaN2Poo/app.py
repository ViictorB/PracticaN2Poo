...
def login():
    if request.method == 'POST':
        ...
        # Verificar si el rol solicitado es "preceptor"
        if rol == 'preceptor':
            # Buscar al preceptor en la base de datos por correo
            preceptor = Preceptor.query.filter_by(correo=correo1).first()
            if preceptor and check_password_hash(preceptor.clave, contraseña):
                # Preceptor autenticado correctamente
                # Puedes realizar redirecciones, establecer una sesión, etc.
                return "Inicio de sesión exitoso como preceptor"
    # Si se accede a la página por GET, mostrar el formulario
